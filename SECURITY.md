...
revoke separately. Signed GitHub App deauthorization invalidates associated Atlas sessions,
devices and pairing codes, rejects older pending sign-ins and turns activity off.
The configured webhook must be activated and its live delivery verified. Duplicate
deliveries cannot invalidate a later reconnection.
...
Report management is restricted to configured numeric moderator identities using
browser sessions. Decisions carry an audit record and exact-snapshot withdrawal
requires separate confirmation. Report prose is rendered as text. Activity is
explicitly opt-in, coarse and limited to a 90-second lease on public Threadwalks;
no location, chamber, private conversation or history is collected.

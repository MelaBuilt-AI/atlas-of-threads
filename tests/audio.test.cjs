...
function player(saved = {}) {
  ...
  vm.runInNewContext(music, {
    document: {getElementById:get, createElement:() => new Element()}, window,
...
test('pause and volume persist; skips stay paused and never change effects preferences', async () => {
  const p = player(); await flush();
  p.get('music-toggle').click();
  p.get('music-next').click(); await flush();
  assert.equal(p.audio.paused, true);
  assert.equal(JSON.parse(p.storage['atlas.music.v1']).paused, true);
  ...
  restored.get('music-toggle').click(); await flush();
